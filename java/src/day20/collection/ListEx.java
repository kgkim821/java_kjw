...
public class ListEx {
	...
	public static void main(String[] args) {
			...
			//1분터 5까지 차레대로 추가
			for(int i =1; i<=5; i++) {
				list.add(i); //정수 i 가 박싱으로 Integer 클래스의 객체가 된 후 list 에 추가
			}
			//1을 더추가
			list.add(1);
			System.out.println(list);
			//1을 제거
			list.remove((Integer)1); //첫번째 만나는 1을 제거
		...
		//일반, 리스트는 특정번지에 접근할 수 있어서 get() 을 이용하여 반복문을 활용할 수 있다.
		for (int i =0 ; i<list.size();i++) {
			System.out.print(list.get(i) + " ");
		}
		System.out.println();
		//향상된 for 문 . List와 set  모두 가능
